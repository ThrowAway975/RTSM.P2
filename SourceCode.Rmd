...
```{r}
#REQUIRED PACKAGES
#f you'ven't installed, run this:
#install.packages(c('tseries','forecast','quantmod','car','FinTS','rugarch'))
...
stock_price=na.omit(stock_data$POWERGRID.NS$POWERGRID.NS.Adjusted)
#pnj_price

#stock_price = POWERGRID.NS$POWERGRID.NS.Close # Adjusted Closing Price
...
```{r}
# Augmented Dickey-Fuller (ADF) Test for Stationarity with POWERGRID Data
# *******************************************************************
...
```{r}
pnj_ds = diff(log(stock_price)); plot(pnj_ds) # POWERGRID (First)return Difference Time-Series
```

```{r}
pnj_ds=na.omit(pnj_ds)
adf_test_pnj_ds = adf.test(pnj_ds); adf_test_pnj_ds # Inference : POWERGRID Difference Time-Series is Stationary

```

```{r}
# Ljung-Box Test for Autocorrelation - POWERGRID Data
# ***********************************************

lb_test_pnj_ds = Box.test(pnj_ds); lb_test_pnj_ds # Inference : POWERGRID Difference (Stationary) Time-Series is Autocorrelated as NULL is rejected and p-value<0.0151 | NULL: No Auto correlation | Alternate: Auto Correlation
```
...
acf(pnj_ds) # ACF of POWERGRID Difference (Stationary) Series
pacf(pnj_ds) # PACF of 1POWERGRID Difference (Stationary) Series
```
...
```{r}
# Lung-Box Test for Autocorrelation - Model Residuals
# ****************************************************
